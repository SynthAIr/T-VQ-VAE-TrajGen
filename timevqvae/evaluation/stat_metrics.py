...
def marginal_distribution_difference(real, generated):
    """
    Calculates the Marginal Distributon Difference (MDD) between real and generated data.
    ...
    x = np.linspace(
        min(real_values.min(), generated_values.min()),
        max(real_values.max(), generated_values.max()),
        100,
    )
...
def auto_correlation_difference(real, generated):
    ...
    generated_acf = np.mean(
        [autocorrelation(series[0]) for series in generated], axis=0
    )
...
if __name__ == "__main__":
    ...
    print(
        ...
        marginal_distribution_difference(real_data, generated_data),
    )
    print(
        "Auto-Correlation Difference:",
        auto_correlation_difference(real_data, generated_data),
    )
