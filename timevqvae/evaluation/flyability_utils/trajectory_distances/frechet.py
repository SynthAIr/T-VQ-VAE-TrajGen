"""
From the repository: https://github.com/bguillouet/traj-dist
"""
...
from .basic_euclidean import (circle_line_intersection, eucl_dist,
                              eucl_dist_traj, point_to_seg)
        ...
        if point_to_seg(p, s[0], s[1], dps1, dps2, ds) > eps:
            ...
            if intersect[0][0] != intersect[1][0] or intersect[0][1] != intersect[1][1]:
                ...
                if px == s1x and py == s1y:
                    ...
                    i1y = intersect[0][1]
                    u1 = (
                        ((i1x - s1x) * (s2x - s1x)) + ((i1y - s1y) * (s2y - s1y))
                    ) / segl2
                    if 0 <= u1 <= 1:
                        lf = [u1, u1]
                    else:
    ...
    for j in range(q):
        for i in range(p - 1):
            LF.update(
                {
                    (i, j): free_line(
                        Q[j], eps, P[i : i + 2], mdist[i, j], mdist[i + 1, j], P_dist[i]
                    )
                }
            )
    BF = {}
    for j in range(q - 1):
        for i in range(p):
            BF.update(
                {
                    (i, j): free_line(
                        P[i], eps, Q[j : j + 2], mdist[i, j], mdist[i, j + 1], Q_dist[j]
                    )
                }
            )
    return LF, BF
...
def compute_critical_values(P, Q, p, q, mdist, P_dist, Q_dist):
    ...
    for i in range(p - 1):
        for j in range(q - 1):
            Lij = point_to_seg(
                Q[j], P[i], P[i + 1], mdist[i, j], mdist[i + 1, j], P_dist[i]
            )
            if Lij > end_point:
                cc.add(Lij)
            Bij = point_to_seg(
                P[i], Q[j], Q[j + 1], mdist[i, j], mdist[i, j + 1], Q_dist[j]
            )
            if Bij > end_point:
                cc.add(Bij)
...
def frechet(P, Q):
    ...
    eps = cc[0]
    while len(cc) != 1:
        m_i = int(len(cc) / 2 - 1)  # list indices must be integers or slices, not float
        eps = cc[m_i]
