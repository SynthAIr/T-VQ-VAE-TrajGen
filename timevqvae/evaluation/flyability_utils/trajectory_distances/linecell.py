...
def linecell_lons_bigger_step(
    p1, p2, cell_start, lons_all, lats_all, lons_center_all, lats_center_all
):
    reverse = False
    if p2[1] < p1[1]:
        tmp = p1
...
def trajectory_grid(traj_0, precision):
    cells_list, lons_all, lats_all, lons_center_all, lats_center_all = (
        trajectory_set_grid([traj_0], precision)
    )
