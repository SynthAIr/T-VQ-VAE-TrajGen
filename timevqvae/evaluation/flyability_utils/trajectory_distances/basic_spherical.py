...
def initial_bearing(lon1, lat1, lon2, lat2):
    ...
    x = math.cos(rad * (lat1)) * math.sin(rad * (lat2)) - math.sin(
        rad * (lat1)
    ) * math.cos(rad * (lat2)) * math.cos(dlon)
...
def cross_track_distance(lon1, lat1, lon2, lat2, lon3, lat3, d13):
    ...
    theta13 = initial_bearing(
        lon1, lat1, lon3, lat3
    )  # bearing from start point to third point
    theta12 = initial_bearing(
        lon1, lat1, lon2, lat2
    )  # bearing from start point to end point
