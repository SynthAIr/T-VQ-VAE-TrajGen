        ...
        for j in range(n_S_old):
            ...
            if (
                ...
                for igp in ranges:
                    pgp = traj_cell_2[igp]
                    if (
                        (p_prec[1] == p[1] and pgp[0] == p[0])
                        or (p_prec[0] == p[0] and pgp[1] == p[1])
                        or igp == ig
                    ):
                        dist_back = (
                            np.linalg.norm(traj_cell_2[igp - 1] - p)
                            if igp != 0
                            else np.inf
                        )
                        dist_forw = (
...
def sowd_grid_brut(traj_cell_1, traj_cell_2):
    sowd_brut_dist = owd_grid_brut(traj_cell_1, traj_cell_2) + owd_grid_brut(
        traj_cell_2, traj_cell_1
    )
