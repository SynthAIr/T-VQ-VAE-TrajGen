...
class MiniRocketTransform(nn.Module):
    ...
    def _generate_kernels(self):
        ...
        for i in range(self.num_kernels):
            kernel = np.random.choice(
                [-1, 2], size=self.kernel_length, p=[2 / 3, 1 / 3]
            )
            if np.sum(kernel) != 0:
    ...
    def _compute_dilations(self, input_length):
        ...
        dilations = np.logspace(
            0, np.log10(max_dilation), num=self.num_kernels, base=2, dtype=int
        )
    ...
    def forward(self, x, normalize=True):
        ...
        for kernel in self.kernels:
            for dilation in self.dilations:
                kernel_dilated = np.zeros(
                    self.kernel_length + (self.kernel_length - 1) * (dilation - 1)
                )
                kernel_dilated[::dilation] = kernel
                kernel_tensor = torch.tensor(
                    kernel_dilated, dtype=torch.float32, device=x.device
                ).view(1, 1, -1)

                conv_output = nn.functional.conv1d(
                    x, kernel_tensor, padding=len(kernel_tensor) // 2
                )
