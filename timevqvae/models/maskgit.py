...
from timevqvae.models import (BidirectionalTransformer, VectorQuantize,
                              VQVAEEncoder)
from timevqvae.utils import (freeze, quantize, zero_pad_high_freq,
                             zero_pad_low_freq)
...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mask_token_ids = {
            "lf": config["VQ-VAE"]["codebook_sizes"]["lf"],
            "hf": config["VQ-VAE"]["codebook_sizes"]["hf"],
        }
        self.gamma = self.gamma_func("cosine")

        # load the staeg1 model
        from timevqvae.trainers import Stage1 # import here to address circular import
        self.stage1 = Stage1.load_from_checkpoint(
            stage1_ckpt_fname,
    ...
    def _randomly_mask_tokens(self, s, mask_token_id, device):
        ...
        n_unmasks = np.clip(n_unmasks, a_min=0, a_max=n - 1).astype(
            int
    ...
    def first_pass(
        ...
        for t in range(self.T["lf"]):
            ...
            masking = self.mask_by_random_topk(
                mask_len,
                selected_probs,
                temperature=self.choice_temperature_l * (1.0 - ratio),
                device=device,
            )
    ...
    def second_pass(
        ...
        for t in range(self.T["hf"]):
            ...
            masking = self.mask_by_random_topk(
                mask_len,
                selected_probs,
                temperature=self.choice_temperature_h * (1.0 - ratio),
                device=device,
            )
    ...
    def critical_reverse_sampling(
        ...
        error_ratio_hist = deque(
            maxlen=round(
                self.T[kind] * self.config["MaskGIT"]["ESS"]["error_ratio_ma_rate"]
            )
        )
        for t in range(1, self.T[kind])[::-1]:
            # masking ratio according to the masking scheduler
            ratio_t = 1.0 * (t + 1) / self.T[kind]  # just a percentage e.g. 1 / 12
            ratio_tm1 = 1.0 * t / self.T[kind]  # tm1: t - 1
            mask_ratio_t = self.gamma(ratio_t)
            mask_ratio_tm1 = self.gamma(ratio_tm1)  # tm1: t - 1

            # mask length
            mask_len_t = torch.unsqueeze(
                torch.floor(unknown_number_in_the_beginning_l * mask_ratio_t), 1
            )
            mask_len_tm1 = torch.unsqueeze(
                torch.floor(unknown_number_in_the_beginning_l * mask_ratio_tm1), 1
            )
            ...
            logits = self.masked_prediction(
                transformer, class_condition, s_tm1
            )  # (b m k)
    ...
    def iterative_decoding_with_self_token_critic(
        ...
        for t in range(t_star, self.T[kind]):
            ...
            # create masking according to `t`
            ratio = 1.0 * (t + 1) / self.T[kind]  # just a percentage e.g. 1 / 12
    ...
    def compute_confidence_score(
        ...
        for n in range(confidence_scores.shape[-1]):
            ...
            logits = self.masked_prediction(
                transformer, class_condition, s_m
            )  # (b n k)
