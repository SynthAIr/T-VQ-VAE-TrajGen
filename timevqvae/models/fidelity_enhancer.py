...
from timevqvae.utils import SnakeActivation
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.final_conv = nn.Sequential(
            nn.Conv1d(dim, self.out_dim, kernel_size=1),
            nn.Conv1d(
                self.out_dim,
                self.out_dim,
                kernel_size=3,
...
class FidelityEnhancer(nn.Module):
    def __init__(self, input_length, in_channels, config):
        ...
        self.register_buffer("tau", torch.tensor(0.0).float())
    ...
    def forward(self, x_a):
        ...
        # x_a = F.upsample(x_a, size=self.input_length, mode='linear', align_corners=False)
        x_a = F.interpolate(
            x_a, size=self.input_length, mode="linear", align_corners=False
        )
        xhat = self.unet(x_a)  # (b 1 l)
