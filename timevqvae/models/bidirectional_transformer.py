...
from x_transformers import Encoder as TFEncoder
...
class BidirectionalTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.tok_emb_l = nn.Embedding(
            codebook_sizes["lf"] + 1, embed_dim
        )  # `+1` is for mask-token
        if kind == "hf":
            self.tok_emb_h = nn.Embedding(
                codebook_sizes["hf"] + 1, embed_dim
            )  # `+1` is for mask-token
    ...
    def forward_lf(self, s_M_l, class_condition: Union[None, torch.Tensor] = None):
        ...
        logits = (
            torch.matmul(embed, self.tok_emb_l.weight.T) + self.bias
        )  # (b, n, codebook_size+1)
        logits = logits[
            :, :, :-1
    ...
    def forward_hf(self, s_M_l, s_M_h, class_condition=None):
        ...
        if self.training:
            token_embeddings_l = self._token_emb_dropout(
                s_M_l, token_embeddings_l, "lf", p=self.emb_dropout
            )
        ...
        logits = (
            torch.matmul(embed, self.tok_emb_h.weight.T) + self.bias
        )  # (b, m, codebook_size+1)
        logits = logits[
            :, :, :-1
        ]  # remove the logit for the mask token.  # (b, m, codebook_size)
        return logits
