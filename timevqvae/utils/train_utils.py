...
def set_seed(seed: int) -> None:
    ...
    torch.cuda.manual_seed_all(seed)  # Set CUDA seed for all GPUs
    torch.backends.cudnn.deterministic = True  # Enable deterministic algorithms
    torch.backends.cudnn.benchmark = False  # Disable dynamic backend selection
...
def quantize(
    z,
    vq_model,
    transpose_channel_length_axes=False,
...
def compute_emb_loss(codebook, x, use_cosine_sim, esm_max_codes):
    ...
    esm_loss = F.mse_loss(cov_x.detach(), cov_embed) + F.mse_loss(
        mean_x.detach(), mean_embed
    )
    return esm_loss
...
def linear_warmup_cosine_annealingLR(
    ...
    assert (
        linear_warmup_rate > 0.0 and linear_warmup_rate < 1.0
